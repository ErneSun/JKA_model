...
This package performs the frozen-V0.6 Residual Structure Assessment on CUDA. It trains probe closures only, preserves the 144-record `3 backbone/data × 3 closure-init` matrix, records `S_R` as a magnitude diagnostic, selects `P_R/G_H` structure decisions on validation, confirms the locked result on test, and emits an R1/R2/R3/INCONCLUSIVE route. Low magnitude never discards a residual; memory class is secondary.
