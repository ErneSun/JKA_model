...
6. Select the model/H by validation residual NRMSE, then evaluate held-out teacher-forced residual prediction separately from closed-loop field/physics metrics and record error/burden curves.
7. Validate checkpoint/data/split/normalizer/evaluation-trajectory identity before comparison.
8. Require within-backbone initialization consistency and then across-backbone consistency. Report residual learnability, closed-loop utility, and memory class separately; write CSV/JSON/plots/Markdown reports plus `completion.json`.
