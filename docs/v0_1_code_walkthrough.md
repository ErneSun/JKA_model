...
> 历史导读说明：仓库运行时现为 V0.4/`0.4.0`。V0.1 smoke 仍作为回归入口，但
...
- 外层 `ProjectConfig(project_version="0.4.0", tags=("v0.1", "smoke"))`
...
| `ProjectConfig` | `project_version` | 当前回归运行必须等于运行时 `0.4.0`。 |
...
| `project_version` | 软件项目版本 | 保证 runtime 兼容 | `"0.4.0"` |
