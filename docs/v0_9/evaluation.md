...
locked test → teacher-free rollout → problem-owned observable decoding → aggregation。
...
- 每个指标的方向、绝对阈值、相对 non-inferiority 与数值分辨率；
- `PASS/FAIL/INCONCLUSIVE`，不把缺失或非有限证据强行二值化。
...
报告同时独立汇总 `operator_explained_residual`、`dynamic_operator_adaptation`、`observable_support`
和原 joint decision。频率误差允许一个预声明 FFT bin 的分辨率下限；其他物理量不共用该容差。

详细合同见 `evaluation_and_observable_revision.md`。
