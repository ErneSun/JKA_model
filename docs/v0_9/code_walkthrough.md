...
7. `adaptive/physics.py` 通过冻结 decoder 实现 raw-unit velocity/vorticity/divergence/no-slip anchor。
8. `train/train_v0_9.py` 只优化 operator adapter，并保存 exact-resume checkpoint 与课程日志。
9. `adaptive/rollout.py` 在 initial context 后完全使用 predicted latent history并记录 trust gate。
10. `eval/evaluate_v0_9.py` 计算 residual decomposition、controls、rollout 和 physical metrics。
11. `adaptive/reporting.py` 先在 operator init 内、再跨 backbone seeds 聚合。
12. `gpu_validate_all.py` 执行受约束 rank selection，是唯一正式 GPU orchestration entrypoint。
