...
Covered contracts include exact residual formula, online-versus-EMA identity, stop-gradient, train-only standardization, raw/normalized metric separation, H=1 paired equivalence, zero-output initialization, no future leakage, frozen ownership, predicted-history rollout, RNG-free analytical parameter matching, shuffled controls, nested seed ownership, validation-only route selection, locked test confirmation, low-magnitude residual retention, all R1–R3 plus inconclusive routes, three-condition physics AND, exact record/provenance checks, completion/failure schemas, checkpoint scale provenance, and CPU cache/train/evaluate integration.
